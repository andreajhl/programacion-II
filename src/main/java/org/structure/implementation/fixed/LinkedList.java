...
public class LinkedList<T> implements LinkedListADT<T> {
    ...
    public void add(T value) {
        if (this.size() == MAX_SIZE) throw new FullADTException();
    ...
    public void insert(int index, T value) {
        if (index < 0 || index > this.size() || index >= MAX_SIZE) throw new IndexOutOfBoundsADTException();
        if (this.size() == MAX_SIZE) throw new FullADTException();
        if (index == this.size()) {
            this.add(value);
            return;
    ...
    public void remove(int index) {
        if (index < 0 || index >= size()) throw new IndexOutOfBoundsADTException();
    ...
    public T get(int index) {
        if (index < 0 || index >= this.size()) throw new IndexOutOfBoundsADTException();
        if (isEmpty()) throw new EmptyADTException();
        Node<T> node = getNode(index);
        if (node == null) throw new ElementNotFoundADTException();
